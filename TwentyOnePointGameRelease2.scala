  ...
  *   玩家要牌
  ...
  def GamerAskForPoker()= {
    ...
    /**
      *返回玩家的分数 这两个函数是新增的
      */
    def getPointOfGamer : Int = (for (i <- 0 to NumOfGamerPoker - 1) yield PokerPoint(Gamer(i))).sum

    /**
      *返回电脑的分数 这两个函数是新增的
      */

    def getPointOfDealer : Int = (for (i <- 0 to NumOfDealerPoker - 1) yield PokerPoint(Dealer(i))).sum
